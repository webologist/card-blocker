...
| ALT-LOG-06 | **Unrecognised number as *Alternate* must not create an account** | Select *Alternate*, enter a number nobody has nominated, verify `1234` | Refused with "This number has not been nominated as an alternate number by anyone. You may choose to register as a new user."; no new record, no welcome messages | ❌→🔧 **Was: registered as a new own account, "Registered" logged, welcome SMS + WhatsApp sent (BUG-20).** Fixed; retested → no record created |
| ALT-LOG-07 | **Unverified alternate must be refused** | Seed `altPhone` with `altVerified: false`; sign in on it as *Alternate* | Refused, and told the nomination exists but was never verified — not told it was never nominated | ❌→🔧 **Was: full access granted (BUG-21).** Fixed; retested → refused |
| ALT-LOG-08 | **Two owners sharing one alternate** | Seed two paid accounts that both nominate `9877700077`; sign in on it as *Alternate* | Choose which account to act for | ❌→🔧 **Was: silently landed on the first account only, the second owner's cards unreachable with nothing indicating the account existed (BUG-22).** Fixed; retested → "Whose cards are you here for?" lists both, and picking the second opens it |
| ALT-LOG-12 | Regression: a single nomination skips the chooser | One account nominates the number; sign in on it as *Alternate* | Straight to that owner's dashboard, no chooser | 🔧 Pass |
| ALT-LOG-13 | Chooser is not a login | On the chooser, press *Go back* | Returns to the login screen; no account entered, no session stored | 🔧 Pass |
