...
// HMAC-sign a token for dummy mode (Verify handles its own tokens in live mode)
...
/**
 * OTP mode resolution — priority order:
 *  1. OTP_MODE env var  (set in Vercel dashboard — overrides everything)
 *  2. req.body.dummyMode (admin toggle stored client-side in cbp:otp_mode)
 */
...
export default async function handler(req, res) {
  ...
  // ── Dummy mode: no Twilio call, fixed OTP "1234" ──────────────────────────
  if (isDummyMode(req.body)) {
    ...
    console.log(`[OTP] DUMMY MODE — skipping Twilio, phone=${fullPhone}`);
  ...
  const serviceSid = process.env.TWILIO_VERIFY_SERVICE_SID; // preferred: Verify service
  const fromPhone  = process.env.TWILIO_PHONE_NUMBER;        // fallback: Messages API

  // If a Verify Service SID is configured, use Twilio Verify (recommended)
  if (serviceSid && accountSid && authToken) {
    try {
      ...
      // Verify manages its own tokens; we return the phone as the session key
  ...
  // Fallback: Messages API with custom OTP (original approach)
  if (accountSid && authToken && fromPhone) {
    ...
    try {
      ...
      if (!twilioRes.ok) {
        console.error('[OTP] Twilio Messages error:', JSON.stringify(data));
        return res.status(500).json({ error: 'Failed to send OTP. Please try again.' });
      }
  ...
  console.error('[OTP] No Twilio credentials configured');
