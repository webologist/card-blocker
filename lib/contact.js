// lib/contact.js
// Shared server-side pieces of the contact form: validation, the email
...
// The only mobile-number gate now that the client-side widget is gone.
...
function buildContactEmail(d, receivedAt) {
  ...
  // "Frm.web" marks these as coming from the site's contact form; the sender's
  // own subject follows so each message stays distinct in the inbox rather
  // than threading together under one repeated subject.
  return { subject: `Frm.web - ${d.subject}`, html, text };
}

// Every contact message goes to the support mailbox that is actually monitored.
// Note this is deliberately not the support@blockmycard.in address published on
// the site - that one is where users write to us; this is where the form's own
// submissions are collected and worked through.
...
const DEFAULT_CONTACT_TO = 'support.blockmycard@gmail.com';
