...
(function () {
  ...
  function fixLogoutButton() {
    ...
    if (!btn) { var old = document.getElementById('bmc-logout-bar'); if (old) old.remove(); return; }
    var label = btn.textContent.trim();
    var phoneEl = rh.querySelector('span.font-mono');
    var phoneText = phoneEl ? phoneEl.textContent.trim() : '';
    var bar = document.getElementById('bmc-logout-bar');
    if (!bar) {
      bar = document.createElement('div'); bar.id = 'bmc-logout-bar';
      bar.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9998;background:#1e293b;display:flex;align-items:center;justify-content:flex-end;padding:.35rem 1rem;gap:1rem;font-family:ui-sans-serif,system-ui,sans-serif;line-height:1;';
      var ph = document.createElement('span'); ph.id = 'bmc-logout-phone';
      ph.style.cssText = 'font-family:monospace;font-size:.75rem;color:#94a3b8;';
      bar.appendChild(ph);
      var proxy = document.createElement('button'); proxy.id = 'bmc-logout-proxy';
      proxy.style.cssText = 'background:none;border:1px solid #475569;color:#cbd5e1;border-radius:.375rem;padding:.2rem .65rem;cursor:pointer;font-size:.75rem;font-weight:600;';
      proxy.onmouseenter = function(){ proxy.style.color = '#fff'; };
      proxy.onmouseleave = function(){ proxy.style.color = '#cbd5e1'; };
      bar.appendChild(proxy);
      document.body.insertBefore(bar, document.body.firstChild);
    }
    var ph2 = document.getElementById('bmc-logout-phone');
    var proxy2 = document.getElementById('bmc-logout-proxy');
    if (ph2) ph2.textContent = phoneText;
    if (proxy2) {
      proxy2.textContent = label;
      proxy2.onclick = function() { btn.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true })); };
    }
  }
  ...
  function showError(msg) {
    ...
    if (!t) {
      t = document.createElement('div'); t.id = 'bmc-otp-toast';
      ...
      x.style.cssText = 'position:absolute;top:.5rem;right:.65rem;background:none;border:none;cursor:pointer;color:#b91c1c;font-size:.9rem;';
      x.onclick = hideError; t.appendChild(x); document.body.appendChild(t);
    }
    var x2 = t.querySelector('button');
    Array.from(t.childNodes).forEach(function(n){ if (n !== x2) t.removeChild(n); });
    t.insertBefore(document.createTextNode(msg), x2);
  ...
  function sanitisePhone(raw) {
    ...
    if (d[0] === '0' && d.length >= 11) d = d.slice(1);
    return d;
  }
      ...
      getDummyMode().then(function(dummy) {
        ...
        .then(function(res) {
          ...
          return safeJson(res).then(function(data) {
            if (!data || !data.token) { showError((data && data.error) || 'Failed to send OTP.'); btn.disabled = false; btn.textContent = orig; return; }
        ...
        .then(function(res) {
          ...
          return safeJson(res).then(function(data) {
            if (data && data.success) {
              ...
              btn.disabled = false; btn.textContent = orig2; btn.click();
            } else {
